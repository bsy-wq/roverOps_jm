...
const QString G_REMOTE_USER = "jianmi";             // 机器人用户名
//const QString G_REMOTE_IP   = "192.168.31.33";  // 机器人IP
const QString G_REMOTE_IP   = "10.42.0.1";  // 机器人IP
  ...
  , sp_ptr(nullptr)
{
  ...
  // =========================================================
  // 【新增】读取上次输入的地图路径
  // "MyRobotApp" 是组织名，"Config" 是应用名，系统会自动在注册表或配置文件里找
  // =========================================================
  QSettings settings("MyRobotApp", "Config");
    
  // 读取键值 "LastMapPath"，如果没有存过，就使用默认值 "/home/jianmi/..."
  QString defaultPath = "/home/jianmi/catkin_pct/src/pct-planner/src/pct_planner_ros/rsc/pcd/building.pcd";
  QString lastPath = settings.value("LastMapPath", defaultPath).toString();
    
  // 显示到输入框里
  ui.lineEdit_MapPath->setText(lastPath);
...
// =========================================================================
// 按钮 1：远程启动 Launch 文件 (roslaunch)
// =========================================================================
void MainWindow::on_pushButton_1_clicked()
{
    // ================= 配置区域 =================
    //QString remoteUser = "jianmi";
    //QString remoteIP   = "192.168.31.33"; 
    // ===========================================

    // 【核心修复 1】 显式指定 ROS_IP
    // 强制告诉远程节点使用 IP 地址通讯，而不是用主机名 "cgw"
    QString exportIp = QString("export ROS_IP=%1").arg(G_REMOTE_IP);

    // 【核心修复 2】 显式指定库路径 (之前的修复保留)
    QString exportLib = "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/home/jianmi/catkin_pct/src/pct-planner/src/pct_planner_ros/lib/3rdparty/gtsam-4.1.1/install/lib";
    
    // 原来的 source 命令
    QString sourceRos = "source /opt/ros/noetic/setup.bash";
    QString sourceWs  = "source /home/jianmi/catkin_pct/devel/setup.bash";

    // 启动命令 (记得加上 rviz:=false 防止远程开启图形界面)
    QString startCmd  = "roslaunch pct_planner global_planner.launch";

    // ===========================================
    // 拼接所有环境变量：
    // 先 export IP && export Lib && source环境 && 启动
    // ===========================================
    QString remoteEnv = QString("%1 && %2 && %3 && %4")
                        .arg(exportIp)  // <--- 加了这一项
                        .arg(exportLib)
                        .arg(sourceRos)
                        .arg(sourceWs);
    ...
    // 下面的代码保持不变
    QString sshCmd = QString("ssh -o StrictHostKeyChecking=no -t %1@%2 '%3 && %4'")
                     .arg(G_REMOTE_USER)
                     .arg(G_REMOTE_IP)
                     .arg(remoteEnv)
                     .arg(startCmd);
    ...
    if (showTerminal)
    {
        ...
        QString fullLocalCmd = QString("bash -c \"%1; exec bash\"").arg(sshCmd);
        arguments << "--" << "bash" << "-c" << fullLocalCmd;
...
{
    // =========================================================
    // 【设置开关】 true = 弹窗; false = 静默
    // =========================================================
    bool showTerminal = false; // 比如这个我不想要弹窗了
    // =========================================================

    //QString remoteUser = "jianmi";
    //QString remoteIP   = "192.168.31.33";
    QString remoteEnv  = "source /opt/ros/noetic/setup.bash";
    QString startCmd   = "roscore";
    QString killCmd    = "killall -9 roscore rosmaster > /dev/null 2>&1";

    QString sshCmd = QString("ssh -t %1@%2 '%3; %4 && %5'")
                     .arg(G_REMOTE_USER).arg(G_REMOTE_IP)
                     .arg(killCmd).arg(remoteEnv).arg(startCmd);
    ...
    {
        // [静默模式]
        // 这里稍微复杂一点，因为 roscore 是一个持续运行的程序。
        // 如果用局部变量 QProcess，函数结束它就被杀掉了。
        // 所以我们需要用 new QProcess，并且还要找个地方存它（如果你以后想停止它）。
        
        // *如果你已经在 MainWindow.h 定义了 QProcess *process 成员变量，就用它*
        // 如果没有，这里演示 "Fire and Forget" (启动了就不管了) 的写法：
        
        QProcess *bgProcess = new QProcess(this); // 指定父对象，窗口关闭时自动回收
        
        // 打印日志
        connect(bgProcess, &QProcess::readyReadStandardOutput, [=](){
             qDebug() << "ROSCORE OUT:" << bgProcess->readAllStandardOutput();
        });
        
        bgProcess->start("bash", QStringList() << "-c" << sshCmd);
        qDebug() << "Roscore started in background...";
    }
}


void MainWindow::on_btn_3d_goal_clicked() // 或者是 on_pushButton_Launch_clicked 等对应的槽函数
{
    // 确保 myviz 指针存在
    if (myviz) {
        // 【修正】使用 Set3DGoalPose 而不是 Activate3DGoalTool
        myviz->Set3DGoalPose(); 
    }
}
...
// =========================================================
// 【新增】FastLIO 初始位姿按钮的实现
// =========================================================
void MainWindow::on_btn_set_init_pose_clicked()
{
    // myviz 是你在 main_window.hpp 里定义的 qrviz 指针对象
    // 如果你的指针名字叫 qrviz_ 或者是其他名字，请在这里对应修改
    if (myviz) 
    {
        // 调用我们在 qrviz.cpp 里写的那个函数
        myviz->SetFastLIOInitialPose();
    }
}
// =========================================================
// 【新增】雷达驱动按钮的实现
// =========================================================
void MainWindow::on_pushButton_lidar_clicked()
{
    // =========================================================
    // 【设置开关】 true = 弹窗显示; false = 后台静默
    // =========================================================
    bool showTerminal = true; 
    // =========================================================

    // 1. 自动生成环境变量 (复制了之前的修复逻辑，保证稳定)
    // 包含: ROS_IP(解决无数据), LD_LIBRARY_PATH(解决缺库), source环境
    QString remoteEnv = QString(
        "export ROS_IP=%1 && "
        //"export LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH && "
        "source /opt/ros/noetic/setup.bash && "
        "source /home/%2/catkin_ws/devel/setup.bash"
    ).arg(G_REMOTE_IP).arg(G_REMOTE_USER);

    // 2. 要运行的命令
    //QString startCmd = "roslaunch livox_ros_driver2 msg_MID360.launch";
    QString startCmd = "roslaunch rslidar_sdk start.launch";
    // 3. 组合 SSH 命令
    // 使用 -o StrictHostKeyChecking=no 跳过第一次连接询问
    QString sshCmd = QString("ssh -o StrictHostKeyChecking=no -t %1@%2 '%3 && %4'")
                     .arg(G_REMOTE_USER)
                     .arg(G_REMOTE_IP)
                     .arg(remoteEnv)
    ...
    else
    {
        ...
        // 捕获输出，方便调试
        connect(bgProcess, &QProcess::readyReadStandardOutput, [=](){
             qDebug() << "[Livox OUT]:" << bgProcess->readAllStandardOutput();
        });
        
        // 自动回收内存
        connect(bgProcess, static_cast<void(QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished),
                [=](int, QProcess::ExitStatus){
            bgProcess->deleteLater();
        });

        bgProcess->start("bash", QStringList() << "-c" << sshCmd);
        qDebug() << "[Livox] Background started.";
    }
}
...
// 【新增】重定位输入框及按钮的实现
// =========================================================
void MainWindow::on_pushButton_RunLoc_clicked()
{
    // =========================================================
    // 1. 获取输入并【记忆】
    // =========================================================
    QString mapPath = ui.lineEdit_MapPath->text().trimmed(); // 去除首尾空格

    if (mapPath.isEmpty()) {
        qDebug() << "Map path is empty!";
        return;
    }

    // 保存到系统配置中，下次打开程序自动加载
    QSettings settings("MyRobotApp", "Config");
    settings.setValue("LastMapPath", mapPath);

    // =========================================================
    // 2. 配置开关 (显示/隐藏终端)
    // =========================================================
    bool showTerminal = true; // 你可以在这里改，或者像之前一样绑定到 CheckBox

    // =========================================================
    // 3. 准备远程环境 (直接用之前的全局变量 G_REMOTE_IP)
    // =========================================================
    QString remoteEnv = QString(
        "export ROS_IP=%1 && "
        //"export LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH && "
        "source /opt/ros/noetic/setup.bash && "
        "source /home/%2/fast_wa/devel/setup.bash"
    ).arg(G_REMOTE_IP).arg(G_REMOTE_USER);

    // =========================================================
    // 4. 拼接启动命令
    // 目标命令：roslaunch ... map:='/path/to/map.pcd'
    // 注意：在 SSH 的单引号字符串里，我们要用双引号 \" 包裹路径，这样最稳妥
    // =========================================================
    QString startCmd = QString("roslaunch fast_lio_localization localization_airy96.launch map:=\"%1\"")
                       .arg(mapPath);
    ...
    if (showTerminal)
    {
        ...
        arguments << "--" << "bash" << "-c" << fullLocalCmd;
        QProcess::startDetached(program, arguments);
        qDebug() << "[Loc] Terminal launched with map:" << mapPath;
    }
    else
    {
        QProcess *bgProcess = new QProcess(this);
        connect(bgProcess, &QProcess::readyReadStandardOutput, [=](){
             qDebug() << "[Loc OUT]:" << bgProcess->readAllStandardOutput();
        });
        connect(bgProcess, static_cast<void(QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished),
                [=](int, QProcess::ExitStatus){
            bgProcess->deleteLater();
        ...
        bgProcess->start("bash", QStringList() << "-c" << sshCmd);
        qDebug() << "[Loc] Background launched with map:" << mapPath;
    }
}
// =========================================================
// 【新增】启动雷达点云三维压二维按钮的实现
// =========================================================
void MainWindow::on_pushButton_3t2_clicked()
{
    // =========================================================
    // 1. 设置开关
    // =========================================================
    bool showTerminal = true; // true=显示终端, false=后台运行

    // =========================================================
    // 2. 脚本路径配置 (!!! 请务必修改这里 !!!)
    // 假设你的脚本在 catkin_ws/src/... 目录下，请填入绝对路径
    // 如果脚本就在主目录下，可以写 "/home/" + G_REMOTE_USER + "/3d_2_2d.py"
    // =========================================================
    QString scriptPath = QString("/home/%1/PCT_planner/run/3d_2_2d.py").arg(G_REMOTE_USER);
    
    // =========================================================
    // 3. 准备环境变量
    // Python 脚本通常也需要加载 ROS 环境 (比如用到 rospy 或 cv_bridge)
    // =========================================================
    QString remoteEnv = QString(
        "export ROS_IP=%1 && "
        //"export LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH && "
    ...
    // =========================================================
    // 4. 拼接启动命令
    // 逻辑：python3 /绝对路径/脚本.py
    // =========================================================
    QString startCmd = QString("python3 %1").arg(scriptPath);
    ...
    if (showTerminal)
    {
        ...
        QProcess::startDetached(program, arguments);
        qDebug() << "[Py Script] Terminal launched: " << scriptPath;
    }
    else
    {
        // [模式 B: 后台静默]
        ...
        // 捕获输出
        connect(bgProcess, &QProcess::readyReadStandardOutput, [=](){
             qDebug() << "[Py OUT]:" << bgProcess->readAllStandardOutput();
        });
        connect(bgProcess, &QProcess::readyReadStandardError, [=](){
             qDebug() << "[Py ERR]:" << bgProcess->readAllStandardError();
        });
        ...
        bgProcess->start("bash", QStringList() << "-c" << sshCmd);
        qDebug() << "[Py Script] Background launched: " << scriptPath;
    }
}
// =========================================================
// 【新增】启动发布base_link按钮的实现
// =========================================================
void MainWindow::on_pushButton_base_clicked()
{
    ...
    // 如果脚本就在主目录下，可以写 "/home/" + G_REMOTE_USER + "/3d_2_2d.py"
    // =========================================================
    QString scriptPath = QString("/home/%1/PCT_planner/run/base_link.py").arg(G_REMOTE_USER);
...
// 【新增】运控模式转发按钮的实现
// =========================================================
void MainWindow::on_pushButton_yk_clicked()
{
    // =========================================================
    // 【设置开关】 true = 弹窗显示; false = 后台静默
    // =========================================================
    ...
    QString remoteEnv = QString(
        ...
        //"export LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH && "
        "source /opt/ros/noetic/setup.bash && "
        "source /home/%2/catkin_pct/devel/setup.bash"
    ).arg(G_REMOTE_IP).arg(G_REMOTE_USER);

    // 2. 要运行的命令
    QString startCmd = "rosrun dds2ros r2d_node";
...
//     // 如果脚本就在主目录下，可以写 "/home/" + G_REMOTE_USER + "/3d_2_2d.py"
//     // =========================================================
//     QString scriptPath = QString("/home/%1/catkin_pct/src/pct-planner/src/pct_planner_ros/scripts/master_controller.py").arg(G_REMOTE_USER);
...
// 【新增】启动楼梯跟随目标点程序按钮的实现
// =========================================================
void MainWindow::on_pushButton_stair_clicked()
{
    // =========================================================
    // 1. 设置开关
    // =========================================================
    ...
    // 如果脚本就在主目录下，可以写 "/home/" + G_REMOTE_USER + "/3d_2_2d.py"
    // =========================================================
    QString scriptPath = QString("/home/%1/PCT_planner/run/Neupan/stair_run.py").arg(G_REMOTE_USER);
...
// =========================================================
// 【新增】启动neupan平地跟随避障程序按钮的实现
// =========================================================
void MainWindow::on_pushButton_flat_clicked()
{
    // =========================================================
    // 1. 设置开关
    // =========================================================
    ...
    // 如果脚本就在主目录下，可以写 "/home/" + G_REMOTE_USER + "/3d_2_2d.py"
    // =========================================================
    QString scriptPath = QString("/home/%1/PCT_planner/run/Neupan/flat_neupan.py").arg(G_REMOTE_USER);
...
void MainWindow::on_pushButton_run_clicked()
{
    ...
    // 如果脚本就在主目录下，可以写 "/home/" + G_REMOTE_USER + "/3d_2_2d.py"
    // =========================================================
    QString scriptPath = QString("/home/%1/PCT_planner/run/cmd_sub_2_lcm.py").arg(G_REMOTE_USER);
...
// 【新增】多点导航四个程序按钮的实现
// =========================================================
void MainWindow::on_pushButton_start_task_clicked()
{
    // =========================================================
    // 【设置开关】 true = 弹窗显示; false = 后台静默
    // =========================================================
    ...
    // 2. 要运行的命令
    //QString startCmd = "roslaunch livox_ros_driver2 msg_MID360.launch";
    QString startCmd = "rosservice call /start_task";
    // 3. 组合 SSH 命令
    // 使用 -o StrictHostKeyChecking=no 跳过第一次连接询问
    QString sshCmd = QString("ssh -o StrictHostKeyChecking=no -t %1@%2 '%3 && %4'")
                     .arg(G_REMOTE_USER)
...
void MainWindow::on_pushButton_toggle_task_clicked()
{
    // =========================================================
    // 【设置开关】 true = 弹窗显示; false = 后台静默
    // =========================================================
    ...
    // 2. 要运行的命令
    //QString startCmd = "roslaunch livox_ros_driver2 msg_MID360.launch";
    QString startCmd = "rosservice call /toggle_task_state_service";
    // 3. 组合 SSH 命令
    // 使用 -o StrictHostKeyChecking=no 跳过第一次连接询问
    QString sshCmd = QString("ssh -o StrictHostKeyChecking=no -t %1@%2 '%3 && %4'")
                     .arg(G_REMOTE_USER)
...
void MainWindow::on_pushButton_clear_task_clicked()
{
    ...
    // 2. 要运行的命令
    //QString startCmd = "roslaunch livox_ros_driver2 msg_MID360.launch";
    QString startCmd = "rosservice call /clear_waypoints";
    // 3. 组合 SSH 命令
    // 使用 -o StrictHostKeyChecking=no 跳过第一次连接询问
    QString sshCmd = QString("ssh -o StrictHostKeyChecking=no -t %1@%2 '%3 && %4'")
                     .arg(G_REMOTE_USER)
...
void MainWindow::on_pushButton_cancel_task_clicked()
{
    // =========================================================
    // 【设置开关】 true = 弹窗显示; false = 后台静默
    // =========================================================
    ...
    // 2. 要运行的命令
    //QString startCmd = "roslaunch livox_ros_driver2 msg_MID360.launch";
    QString startCmd = "rosservice call /cancel_task";
    // 3. 组合 SSH 命令
    // 使用 -o StrictHostKeyChecking=no 跳过第一次连接询问
    QString sshCmd = QString("ssh -o StrictHostKeyChecking=no -t %1@%2 '%3 && %4'")
                     .arg(G_REMOTE_USER)
